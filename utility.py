...
def get_model_dir(FLAGS):
    return os.path.join(FLAGS.rootpath, FLAGS.train_collection, 'Models-noScore', FLAGS.model_name, get_vocab_name(FLAGS.word_cnt_thr), FLAGS.vf_name)

def get_variable_dir(FLAGS):
    return os.path.join(get_model_dir(FLAGS), 'variables-noScore')

def get_pred_dir(FLAGS):
    output = 'autocap'
    return os.path.join(FLAGS.rootpath, FLAGS.test_collection, output, FLAGS.test_collection, FLAGS.train_collection, FLAGS.model_name, get_vocab_name(FLAGS.word_cnt_thr), FLAGS.vf_name, 'bs%d' % FLAGS.beam_size)

def get_sim_dir(FLAGS):
    return os.path.join(FLAGS.rootpath, FLAGS.val_collection, 'SimilarityIndex-noScore', FLAGS.val_collection, FLAGS.train_collection, FLAGS.model_name,
